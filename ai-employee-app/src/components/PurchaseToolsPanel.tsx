...
import { Clock, Zap, Download, X, MessageSquare, Plus, Trash2 } from 'lucide-react'
import type { PharmacyMessage, PharmacyConversation } from './PharmacyPurchaseView'
...
export default function PurchaseToolsPanel({
  conversation,
  conversations,
  onNew,
  onSwitch,
  ...
  const logs = useMemo(() => buildWorkLogs(messages), [messages])
  const outputs = useMemo(() => buildOutputs(messages), [messages])
  ...
  return (
    <div className="flex h-full w-full flex-col border-l border-border-subtle bg-bg-elevated shadow-lg">
      {/* 头部：采购兔信息 */}
      <div className="flex h-14 shrink-0 items-start justify-between border-b border-border-subtle bg-bg-elevated px-4 pt-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col overflow-hidden">
            <div className="truncate text-base font-semibold text-text-primary">采购兔</div>
            <div className="truncate text-xs text-text-muted">采购省心更省钱</div>
          </div>
        </div>
        {onClose && (
          <button
...
/** 从采购会话消息生成工作日志时间线 */
function buildWorkLogs(messages: PharmacyMessage[]): WorkLogItem[] {
...
function buildOutputs(messages: PharmacyMessage[]): OutputItem[] {
...
/** 根据消息内容推断产出物标题 */
function outputTitle(msg: PharmacyMessage): string {
  const content = msg.content
  if (content.includes('采购方案') || content.includes('三个方案')) return '采购方案'
  if (content.includes('补货')) return '补货建议'
  if (content.includes('集采')) return '集采计划'
  if (content.includes('比价') || content.includes('价格')) return '比价分析'
  if (content.includes('供应商')) return '供应商推荐'
  if (msg.dispatchAgent) return `${msg.dispatchAgent.name}产出`
  return 'AI 产出文档'
}
