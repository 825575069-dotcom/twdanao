...
def prompt_list(request: HttpRequest):
    ...
    if ptype:
        if ptype not in ('home', 'chat'):
            return api_error(code=API_CODE.BAD_REQUEST, msg='type 参数非法（应为 home 或 chat）')
    ...
    return api_success(PromptSerializer(qs, many=True).data)


@api_view(['POST'])
@permission_classes([IsAuthenticated])
@require_permission('prompts.manage')
def prompt_create(request: HttpRequest):
    """POST /api/v1/prompts/create — 新增提示词"""
...
def prompt_detail(request: HttpRequest, pk: int):
    ...
    if request.method == 'GET':
        return api_success(PromptSerializer(prompt).data)

    # 修改 / 删除需要权限
    tenant = Tenant.objects.first()
    if not has_permission(request.user, tenant, 'prompts.manage'):
